...
def add_product(products, name, desc, price, quantity):
    max_id = max(products, key=lambda x: x['id'])   # lambda är en funktion utan namn som i detta fall hämtar nyckels värde från dict

    
    id_value = max_id['id']      # id_value är störtsta id:t i hela db
...
while True:
    try:
        ...
        choice = input("Vill du (V)isa, (T)a bort en produkt eller (L)ägga till en produkt? ").strip().upper()
        
        if choice == "L":     #lägg till
            if 1 <= max <= len(products):
                add_product = products['id'] 
                new_id = f"{index}{ products['name']} } 
